...
class Gym:
    ...
    def _set_ustream(self):
        ...
        if droplanes:
            droplanes = np.array(droplanes, dtype=bool)
            if droplanes.shape != (12,):
                raise ValueError("droplanes expected shape (12,), got", droplanes.shape)
            count   = 12 - np.sum(droplanes)
            if count < 7:
                raise ValueError("Can't drop more than 5 lanes!")
            self._traffic = np.ones(12, dtype=float) * (self._volume / count)
            self._traffic[droplanes] = 0
        else:
    ...
    def __init__(self, simcfg=None, maxtime=None, ustream=None, dstream=None, droplanes=None, nsensors=5):
        """Args
    ...
    def reset(self):
        ...
        self.sumo.set_car_prob(self._get_ustream)
        self.sumo.set_speed_slowdown(self._get_dstream)
    ...
    @property
    def done(self):
        self.collisions > 0 or self.simtime >= self.endtime
