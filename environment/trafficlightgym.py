...
class TrafficLightRewards(object):
    ...
    def move_veh_before_junction_west(self, action):
        ...
        for lane in range(3):
            for queue_length in range(self.queue_length):
                if queue_length == 0 and self.lane_before_junction[3, lane, 0] == 1 and self.traffic_state[traffic_number] == 1:
                    self.lane_before_junction[3, lane, 0] = 0 # vehicle enters intersection
                    self.intersection[lane + 3, 5] += 1
                ...
                elif queue_length > 0 and self.lane_before_junction[3, lane, queue_length] == 1 and self.lane_before_junction[3, lane, queue_length - 1] == 0:
                    self.lane_before_junction[3, lane, queue_length - 1] = 1 # vehicle moves forward in lane
                    self.lane_before_junction[3, lane, queue_length] = 0
                    self.reward_for_lane_movement[action] += self.upstream_status[3]
                if self.traffic_state[traffic_number] == 0:
                    self.penalty_for_non_lane_movement[action] -= (self.lane_before_junction[3, lane, :].sum())*self.upstream_status[3]
            traffic_number -= 1
...
if __name__ == "__main__":
    ...
    agent = TrafficLightGym(queue_length, traffic_rate_upstream, traffic_rate_downstream)
